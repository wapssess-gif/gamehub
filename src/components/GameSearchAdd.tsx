...
export function GameSearchAdd() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error("Поиск не удался");
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {loading && <p className="text-sm text-black/60 dark:text-white/60">Поиск...</p>}
        ...
        <ul className="flex flex-col gap-2">
          {visibleResults.map((game) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-black/60 dark:text-white/60">
                    {game.releaseDate ?? "дата неизвестна"} · {game.genres.join(", ") || "жанр неизвестен"}
                  </p>
                </div>
              </div>
              <button
              ...
              >
                {isPending && addingId === game.externalId ? "Добавляю..." : "Добавить"}
              </button>
            </li>
          ))}
        </ul>
